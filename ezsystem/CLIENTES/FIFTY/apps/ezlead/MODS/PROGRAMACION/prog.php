    ...
    <modseg  id="modprosp">
        ...
        <section id="section1" class="sectionmod">
            ...
            <div class="tabcpo" >
                ...
                <input  type="text" id="proced"  class="input" value="<?php echo $procedimiento0;?>" readonly>
                ...
                <form class="tabcpo" method="POST"  action="saveTRA.php" autocomplete="off" onkeydown="return event.key != 'Enter';" >
                    ...
                    <input hidden type="text" class="input" value="<?php echo $procedimiento0;?>" name="procedimiento">

                

                    <div for="telefono" class="label">Fecha Estimada de Programación</div>
                    <input type="date" id="fechapro" name="fechapro" class="input" required>

                    
                    <div for="telefono" class="label">Diagnóstico</div>
                    <input type="text" id="diagnostico" name="diagnostico" class="input" required>

                    <div for="whats" class="label">Nombre de Paciente</div>
                    <input type="text" id="nombrepx" name="nombrepx" class="input" value=""  required>

                    <div for="email" class="label">Teléfono de Contacto</div>
                    <input type="text" id="tel1" name="tel1" class="input" value=""  required>

                    
                    <div for="obs" class="label">Observaciones</div >
                    <textarea  id="obs"  name="obs" class="textarea justify" spellcheck="false" ><?php echo $Pobs;?></textarea>
                    ...
                    <select id="useresp"  name="etapa" class="select left" required>
                        <option value="" selected>POR CONCILIAR</option>
                        <option value="" ></option>
                        <option value="4" >CONCILIACION</option>       
                        <option value="10" >CANCELAR</option>
                    </select>

                    <input type="submit" value="GUARDAR" id="savemod" name="savepCONC">
                    ...
                    <div class="namecotasky left">Bitácora de Actividades</div>
                        ...
                        <div id="backtab">
                                ...
                                <?php
                                    ...
                                    if($adminrol== $radmin ){
                                        $bdpros= "SELECT * FROM $tabtaskY WHERE idmin = '$Pid0' AND tipominuta = '3' AND app = 'lead' AND activo = 'SI' ";
                                        $qbdpros = $db_task->query($bdpros);
                                    }elseif($adminrol== $radmin ){
                                        $bdpros= "SELECT * FROM $tabtaskY WHERE region LIKE '%$userAcroregion%'AND idmin = '$Pid0' AND tipominuta = '3' AND app = 'lead' AND activo = 'SI' ";
                                        $qbdpros = $db_task->query($bdpros);
                                    }else{
                                        $bdpros= "SELECT * FROM $tabtaskY WHERE useresp LIKE '%$userLog%' AND idmin = '$Pid0' AND tipominuta = '3' AND app = 'lead' AND activo = 'SI' ";
                                        $qbdpros = $db_task->query($bdpros);
                                    }
                ...
                <div id="backahistorias">
                    ...
                    <div class="namecotasky left">Bitácora de Comentarios</div>
                        ...
                        <div id="backtab">
                                ...
                                if($adminrol== $radmin ){
                                    $bdpros= "SELECT * FROM $hist_coment WHERE idtarjeta = '$Pid0' AND etapa = '3' AND activo = '1' ";
                                    $qbdpros = $db_leadplus->query($bdpros);
                                }elseif($adminrol== $radmin ){
                                    $bdpros= "SELECT * FROM $hist_coment WHERE region LIKE '%$userAcroregion%'AND idtarjeta = '$Pid0' AND etapa = '3' AND activo = '1' ";
                                    $qbdpros = $db_leadplus->query($bdpros);
                                }else{
                                    $bdpros= "SELECT * FROM $hist_coment WHERE useresp LIKE '%$userLog%' AND idtarjeta = '$Pid0'  AND etapa = '3' AND activo = '1' ";
                                    $qbdpros = $db_leadplus->query($bdpros);
                                }
