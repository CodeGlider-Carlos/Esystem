...
if (isset($_POST['savenuevoprosp'])){
    ...
    $insertar = "INSERT INTO $crm_pros VALUES (NULL,'$region','$unidad','$categoria','$socio',
    '$titulo.','$nombre', '$especialidad','$rol','$tel1','$tel2','$email','MNX','$fuente','$finterna',
    '$useresp','$obs','0', '0','$userLog','$fechahoy','1')";
    ...
    if(!empty($region)){
        ...
        $update0a = "UPDATE $crm_pros SET categoria='$region' WHERE id='$idupd' ";
    ...
    if(!empty($unidad)){
        ...
        $update0b = "UPDATE $crm_pros SET categoria='$unidad' WHERE id='$idupd' ";
    ...
    if(!empty($categoria)){
        ...
        $update = "UPDATE $crm_pros SET categoria='$categoria' WHERE id='$idupd' ";
        $qsqve = $dbc->query($update);
        
    }
    if(!empty($socio)){

        $update1 = "UPDATE $crm_pros SET socio='$socio' WHERE id='$idupd' ";
    ...
    if(!empty($titulo)){
        ...
        $update2 = "UPDATE $crm_pros SET titulo='$titulo' WHERE id='$idupd' ";
        $qsqve2 = $dbc->query($update2);

    }
    if(!empty($nombre)){
        $update3 = "UPDATE $crm_pros SET nombre='$nombre' WHERE id='$idupd' ";
        $qsqve3 = $dbc->query($update3);
    }else{
        
    }
    if(!empty($especialidad)){
        $update4 = "UPDATE $crm_pros SET especialidad='$especialidad' WHERE id='$idupd' ";
        $qsqve4 = $dbc->query($update4);
    }
    if(!empty($rol)){
        $update4b = "UPDATE $crm_pros SET rol='$rol' WHERE id='$idupd' ";
        $qsqve4b = $dbc->query($update4b);
    }
    if(empty($tel1)){
      
    }else{
        $update5 = "UPDATE $crm_pros SET tel1='$tel1' WHERE id='$idupd' ";
        $qsqve5 = $dbc->query($update5);
    }
    if(!empty($tel2)){
        $update6 = "UPDATE $crm_pros SET tel2='$tel2' WHERE id='$idupd' ";
        $qsqve6 = $dbc->query($update6);
    }
    if(!empty($email)){
        $update7 = "UPDATE $crm_pros SET email='$email' WHERE id='$idupd' ";
        $qsqve7 = $dbc->query($update7);
    }
    if(!empty($fuente)){
        $update8 = "UPDATE $crm_pros SET fuente='$fuente' WHERE id='$idupd' ";
        $qsqve8 = $dbc->query($update8);
    }
    if(!empty($finterna)){
        $update9 = "UPDATE $crm_pros SET finterna='$finterna' WHERE id='$idupd' ";
        $qsqve9 = $dbc->query($update9);
    }
    if(!empty($useresp)){
        $update10 = "UPDATE $crm_pros SET useresp='$useresp' WHERE id='$idupd' ";
        $qsqve10 = $dbc->query($update10);
    }
    if(!empty($obs)){
        $update11 = "UPDATE $crm_pros SET obs='$obs' WHERE id='$idupd' ";
...
<body>
    ...
    <form method="POST" action="prospect_med.php" id="backregresar">
